...
async function handleSubmit(e) {
    ...
    try {
        ...
        if (cursorEl && cursorEl.parentNode) cursorEl.remove();
        if (markdownBodyEl) {
            markdownBodyEl.innerHTML = `<span style="color: #fca5a5;"><strong>Consultation Notice:</strong> ${escapeHTML(err.message || "Connection error during consultation stream.")}</span>`;
        }
    } finally {
        isGenerating = false;
...
function renderSourcesTray(sources) {
    const chipsHtml = sources.map((s, idx) => {
        ...
        const pageStr = escapeHTML(String(s.page_number || 'N/A'));
        return `
            <button class="source-chip" data-idx="${idx}" title="${escapeHTML(s.source_book)} - Page ${pageStr}">
...
// Convert [1], [2] into clickable citation pills and safely render sanitized markdown
function formatMessageWithCitations(text, sources) {
    if (!text) return "";

    // Pre-escape raw HTML tags from user or model output to prevent injection attacks,
    // while preserving markdown formatting (*, _, #, -, `, etc.)
    const safeText = String(text)
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;");

    // Convert [1], [2] citations into interactive citation pills
    const processed = safeText.replace(/\[(\d+)\]/g, (match, p1) => {
        const idx = parseInt(p1, 10);
        return `<a class="inline-citation" data-source-idx="${idx - 1}" href="#citation-${idx}" role="button">[${idx}]</a>`;
    });

    let renderedHtml = "";
    if (typeof marked !== "undefined" && marked && typeof marked.parse === "function") {
        renderedHtml = marked.parse(processed);
    } else {
        renderedHtml = processed.replace(/\n/g, "<br>");
    }
...
function showCitationPopover(source, anchorEl) {
    ...
    const rawUrl = (source.download_url || '').trim();
...
function sanitizeHTML(html) {
    ...
    const ALLOWED_TAGS = [
        ...
        'tbody', 'tr', 'th', 'td', 'a', 'span', 'div'
    ];
    const ALLOWED_ATTRS = ['href', 'target', 'rel', 'class', 'data-source-idx', 'data-idx', 'title', 'role'];
            ...
            for (const child of children) {
                ...
                for (const attr of attrs) {
                    ...
                    if (name === 'href') {
                        ...
                        const isSafeProtocol = val.startsWith('http://') || val.startsWith('https://') || val.startsWith('mailto:') || val.startsWith('#');
                        if (!isSafeProtocol) {
                            child.removeAttribute(attr.name);
                        }
                    }
...
const FALLBACK_CLASSICAL_BOOKS = [
    { title_telugu: "చరక సంహిత - సూత్ర స్థానము", title_english: "Charaka Samhita - Sutra Sthana", category: "ఉప వేదాలు", pages: 412, size_mb: 28, download_url: "https://pdf.freegurukul.org/s/1420", is_ayurveda: true },
    { title_telugu: "చరక సంహిత - చికిత్సా స్థానము", title_english: "Charaka Samhita - Chikitsa Sthana", category: "ఉప వేదాలు", pages: 580, size_mb: 36, download_url: "https://pdf.freegurukul.org/s/1423", is_ayurveda: true },
    { title_telugu: "సుశ్రుత సంహిత - శారీర స్థానము", title_english: "Sushruta Samhita - Sharira Sthana", category: "ఉప వేదాలు", pages: 390, size_mb: 24, download_url: "https://pdf.freegurukul.org/s/1421", is_ayurveda: true },
    { title_telugu: "అష్టాంగ హృదయము - సూత్ర స్థానము", title_english: "Ashtanga Hridaya - Sutra Sthana", category: "ఉప వేదాలు", pages: 320, size_mb: 21, download_url: "https://pdf.freegurukul.org/s/1425", is_ayurveda: true },
    { title_telugu: "అష్టాంగ హృదయము - చికిత్స,కల్ప స్థానము", title_english: "Ashtanga Hridaya - Chikitsa & Kalpa Sthana", category: "ఉప వేదాలు", pages: 460, size_mb: 30, download_url: "https://pdf.freegurukul.org/s/1427", is_ayurveda: true },
    { title_telugu: "అందరికి ఆయుర్వేదం - స్వదేశీ వనములికా వేదం", title_english: "Andariki Ayurvedam - Swadeshi Vanamulika Vedam", category: "ఆయుర్వేదం", pages: 280, size_mb: 18, download_url: "https://pdf.freegurukul.org/s/1406", is_ayurveda: true },
    { title_telugu: "అందరికి ఆయుర్వేదం - స్వదేశీ ఆహార వేదం", title_english: "Andariki Ayurvedam - Swadeshi Ahara Vedam", category: "ఆయుర్వేదం", pages: 240, size_mb: 16, download_url: "https://pdf.freegurukul.org/s/1407", is_ayurveda: true },
    { title_telugu: "గృహ వైద్యం", title_english: "Griha Vaidyam - Household First Aid", category: "ఆయుర్వేదం", pages: 195, size_mb: 12, download_url: "https://pdf.freegurukul.org/s/1414", is_ayurveda: true },
    { title_telugu: "వంట ఇల్లే వైద్యశాల", title_english: "Vanta Ille Vaidyashala - Kitchen Pharmacy", category: "ఆయుర్వేదం", pages: 210, size_mb: 14, download_url: "https://pdf.freegurukul.org/s/1394", is_ayurveda: true },
    { title_telugu: "ప్రకృతి వైద్యం", title_english: "Prakruti Vaidyam - Nature Therapy & Hygiene", category: "ఆయుర్వేదం", pages: 260, size_mb: 17, download_url: "https://pdf.freegurukul.org/s/1390", is_ayurveda: true },
    { title_telugu: "గృహౌషద వనము", title_english: "Grihaushadha Vanamu - Medicinal Herbal Gardens", category: "ఆయుర్వేదం", pages: 180, size_mb: 11, download_url: "https://pdf.freegurukul.org/s/1396", is_ayurveda: true },
    { title_telugu: "మన్కి మిన్కి - ఆయుర్వేదం", title_english: "Manki Minki - Pediatric Ayurvedic Care", category: "ఆయుర్వేదం", pages: 225, size_mb: 15, download_url: "https://pdf.freegurukul.org/s/1418", is_ayurveda: true }
];
...
function initCatalogExplorer() {
    ...
    fetch(apiBase + "/api/books/categories")
        .then(r => {
        ...
        .catch(err => {
            ...
            if (categorySelect && categorySelect.children.length <= 1) {
                categorySelect.innerHTML = `
                    ...
                    <option value="ఉప వేదాలు">ఉప వేదాలు (Vedic Treatises)</option>
                    <option value="ఆయుర్వేదం">ఆయుర్వేదం (Classical Ayurveda)</option>
                `;
            }
        });
    ...
    function renderBooks(books) {
        ...
        resultsGrid.innerHTML = books.map(b => {
            ...
            const rawUrl = (b.download_url || '').trim();
            const isSafeUrl = rawUrl.startsWith('http://') || rawUrl.startsWith('https://');
            const downloadBtn = isSafeUrl ? `<a href="${escapeHTML(rawUrl)}" target="_blank" rel="noopener noreferrer" style="color: #d4af37; margin-left: auto; text-decoration: none; font-size: 0.78rem;" title="Download FreeGurukul PDF"><i class="fa-solid fa-download"></i> PDF</a>` : '';
            return `
                <div class="scripture-chip" style="display: flex; align-items: center; justify-content: space-between; gap: 0.5rem; text-align: left;">
                    <div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                        ...
                        ${ayurBadge}
                        <div style="font-size: 0.72rem; color: var(--text-muted);">${escapeHTML(b.title_english || b.category)} &bull; ${b.pages || 100} pgs (${b.size_mb || 10} MB)</div>
                    </div>
                    ${downloadBtn}
                </div>
            `;
    ...
    async function loadCatalogBooks() {
        ...
        const url = `${apiBase}/api/books?query=${encodeURIComponent(query)}&category=${encodeURIComponent(category)}${catalogAyurvedaOnly ? '&is_ayurveda=true' : ''}&limit=18`;
        ...
        try {
            ...
            console.warn("Catalog load notice, using local classical compendium:", err);
            // Resilient fallback for static hosting / waking backend
            let fallback = FALLBACK_CLASSICAL_BOOKS;
            if (query) {
                fallback = fallback.filter(b => 
                    b.title_telugu.toLowerCase().includes(query) ||
                    (b.title_english && b.title_english.toLowerCase().includes(query)) ||
            ...
            if (statusBar) {
                statusBar.textContent = `Displaying ${fallback.length} classical treatises (Verified Library). Live server search sync available.`;
            }
    ...
    searchInput.addEventListener("input", () => {
        ...
        catalogDebounceTimer = setTimeout(loadCatalogBooks, 250);
    });
